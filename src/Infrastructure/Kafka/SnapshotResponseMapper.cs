...
/// <summary>
/// Adapts our domain notification onto the org shared-library <see cref="SnapshotResponse"/>.
/// The outbound twin of <see cref="SnapshotRequestMapper"/>, and the seam that keeps the org
/// type from leaking past Infrastructure: at lift-and-shift the org DTO becomes a NuGet type
/// and only this mapper changes.
///
/// Everything the org contract carries as text is rendered here, never in the Domain:
/// the status enum becomes "Receiving"/"Complete"/"Failed", timestamps become ISO-8601
/// round-trip ("O") UTC, and an absent timestamp becomes the empty string the org class
/// itself defaults to.
/// </summary>
internal static class SnapshotResponseMapper
{
    ...
    /// <summary>
    /// Round-trip ("O") format, invariant culture. Tracking timestamps are stored as UTC but
    /// come back from SQL as <see cref="DateTimeKind.Unspecified"/>, which "O" would render
    /// without an offset — so the kind is asserted before formatting and the wire value
    /// always carries its <c>Z</c>.
    /// </summary>
    private static string ToWireTimestamp(DateTime value)
    {
