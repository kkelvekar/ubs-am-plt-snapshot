...
.SYNOPSIS
  ...
  tools/UBS.AM.PLT.SnapshotWriter.TestProducer/SnapshotSimulationPublisher.cs).
...
.NOTES
  ...
  Preconditions:
    ...
    - az login already authenticated (worker uses DefaultAzureCredential /
      ...
      Gen2 container configured in src/UBS.AM.PLT.SnapshotWriter.Worker/appsettings.json
      — no Azurite/local SQL container needed, matching fault-injection.ps1's
      TC-13..TC-20 live tests).
    - Worker running: dotnet run --project src/UBS.AM.PLT.SnapshotWriter.Worker
