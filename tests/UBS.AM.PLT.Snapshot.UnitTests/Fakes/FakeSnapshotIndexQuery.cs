...
/// <summary>
/// In-memory <see cref="ISnapshotIndexQuery"/> for the snapshot-detail handler tests:
...
/// </summary>
internal sealed class FakeSnapshotIndexQuery : ISnapshotIndexQuery
{
    public Dictionary<string, string> AdlsPaths { get; } = new(StringComparer.Ordinal);

    public List<string> AdlsPathLookups { get; } = [];

    public Task<IReadOnlyList<SnapshotIndexRow>> QueryAsync(SnapshotGridFilter filter, CancellationToken cancellationToken)
        => Task.FromResult<IReadOnlyList<SnapshotIndexRow>>([]);
