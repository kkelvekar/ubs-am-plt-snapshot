...
5. If the reviewer returns `CHANGES_REQUESTED`, route every item under `Findings` and never route `Suggestions`. Send `level: code` findings to `snapshot-developer`; after each developer fix, require an updated implementation summary and invoke `snapshot-reviewer` again.
6. Route every `level: design` or `level: acceptance-contract` finding to `snapshot-planner` and stop progression until the planner returns a fresh `READY_FOR_IMPLEMENTATION` plan. Then invoke `snapshot-developer` with the fresh plan and invoke `snapshot-reviewer` again before continuing.
...
- Require both tester modes for application slices. For a repository-customization-only slice, pass the planner-selected bounded static evidence to the tester instead of inventing Modes A and B.
